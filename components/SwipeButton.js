...
const SwipeButton = ({ height = exports.DEFAULT_HEIGHT, width = DEFAULT_WIDTH, borderRadius = DEFAULT_BORDER_RRADIUS, title, titleContainerProps, titleProps, titleContainerStyle, titleStyle, completeThresholdPercentage = DEFAULT_COMPLETE_THRESHOLD_PERCENTAGE, underlayStyle, disabled, Icon, containerStyle, circleBackgroundColor, goBackToStart = DEFAULT_GO_BACK_TO_START, onComplete, onSwipeEnd = () => { }, onSwipeStart = () => { }, }) => {
    ...
    const animateToStart = () => {
        react_native_1.Animated.spring(translateX, { toValue: 0, tension: 10, friction: 5, useNativeDriver: false }).start();
        return setEndReached(false);
    };
    const animateToEnd = () => {
        onComplete();
        react_native_1.Animated.spring(translateX, { toValue: scrollDistance, tension: 10, friction: 5, useNativeDriver: false }).start();
        if (goBackToStart) {
            setEndReached(true);
    ...
    const onMove = (_, gestureState) => {
        ...
        if (gestureState.dx < 0 || gestureState.dx > scrollDistance) {
            return react_native_1.Animated.event([{ dx: translateX }], { useNativeDriver: false })(Object.assign(Object.assign({}, gestureState), { dx: gestureState.dx < 0 ? 0 : scrollDistance }));
        }
        return react_native_1.Animated.event([{ dx: translateX }], { useNativeDriver: false })(gestureState);
    };
    const onRelease = () => {
        if (disabled) {
            return;
    ...
    const panResponser = () => react_native_1.PanResponder.create({
    ...
    return (react_1.default.createElement(react_native_1.View, { style: [
        ...
        react_1.default.createElement(SwipeButtonCircle_1.default, { circleBackgroundColor: circleBackgroundColor, Icon: Icon, opacity: opacity, panHandlers: panResponser().panHandlers, translateX: translateX, borderRadius: borderRadius, height: height })));
};
